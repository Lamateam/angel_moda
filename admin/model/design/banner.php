	...
	public function addBanner($data) {
		...
		if (isset($data['banner_image'])) {
			foreach ($data['banner_image'] as $banner_image) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "banner_image SET banner_id = '" . (int)$banner_id . "', end_date = '" . (isset($banner_image['end_date'])) ? $this->db->escape($banner_image['end_date']) : 'NOW()' . "', `has_end_date` = '" . (isset($banner_image['has_end_date']) ? (int)$banner_image['has_end_date'] : 0) . "', `is_html` = '" . (isset($banner_image['is_html']) ? (int)$banner_image['is_html'] : 0) . "', link = '" .  $this->db->escape($banner_image['link']) . "', image = '" .  $this->db->escape($banner_image['image']) . "', sort_order = '" . (int)$banner_image['sort_order'] . "'");
	...
	public function getBannerImages($banner_id) {
		...
		foreach ($banner_image_query->rows as $banner_image) {
			...
			$banner_image_data[] = array(
				...
				'has_end_date'						 => $banner_image['has_end_date'],
				'end_date'								 => $banner_image['end_date'],
				'is_html'									 => $banner_image['is_html']
			);
		}
