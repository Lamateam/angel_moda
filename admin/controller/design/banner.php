...
class ControllerDesignBanner extends Controller {
	...
	protected function getForm() {
		...
		$data['entry_has_end_date'] = $this->language->get('entry_has_end_date');
		$data['entry_end_date'] = $this->language->get('entry_end_date');
		$data['entry_is_html'] = $this->language->get('entry_is_html');
		...
		foreach ($banner_images as $banner_image) {
			...
			if (isset($banner_image['end_date'])) {
				$ed = $banner_image['end_date'];
			} else {
				$ed = "";
			}
			
			if (isset($banner_image['is_html'])) {
				$ih = $banner_image['is_html'];
			} else {
				$ih = 0;
			}
			
			if (isset($banner_image['has_end_date'])) {
				$hed = $banner_image['has_end_date'];
			} else {
				$hed = 0;
			}
		...
		$this->response->setOutput($this->load->view('design/banner_form_extended.tpl', $data));
	}
